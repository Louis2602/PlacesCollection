...
const StyledTypography = styled(Typography)(({ theme }) => ({
	...
	'&:after': {
		...
		borderImage:
			'linear-gradient(to right bottom, var(--purple--color), var(--pink--color)) 1',
		transform: 'scaleX(0)',
...
function shadow({ trigger, mode }) {
	const valid = trigger;
	return {
		boxShadow: `${
			valid
				? `5px 0px 27px -5px ${
						mode ? 'var(--white--color)' : 'var(--black--color)'
				  }`
				: 'none'
		}`,
	};
...
const NavBar = () => {
	...
	return (
		<StyledAppBar style={shadow({ trigger, mode })}>
					...
					>
						{userMenu.map((userItem, idx) => (
							<Grow
								key={idx}
								in={openUser}
								{...(openUser ? { timeout: 600 * idx } : {})}
							>
								<StyledLink
									to={userItem.link}
									onClick={() => handleUserMenu(userItem.obj)}
								>
